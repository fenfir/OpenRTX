...
/* Beep / keytone.  On the HD2 the beep is a PCM square-wave tone generated by
 * the voice-prompt player (core/voicePrompts.c) through the codec-DAC output
 * stream -- the PCM stream is what keeps the codec DAC converting/clocked, and
 * the tone is inaudible otherwise (a hardware PWM tone alone is silent unless a
 * PCM stream is already clocking the DAC).  So these platform hooks are no-ops
 * on this target; the player owns the tone. */
