 ...
 * once the bridge is armed (PCM_MODE=3, VOICE_PATH bit0 + playback bit) the
...
void streamHalt()
{
    SOCSYS->VOICE_PATH = st.vpSave; // disarming stops the frame IRQs
    if(st.ctx != nullptr)
        st.ctx->running = 0;
                        ...
                        struct streamCtx *ctx)
{
    ...
    st.vpSave = SOCSYS->VOICE_PATH;
    SOCSYS->SYS_SOFT_RSTN |= SOFT_RSTN_PCM_BITS;
    SOCSYS->PCM_MODE       = 3u;
    SOCSYS->VOICE_PATH    |= (VOICE_PATH_PCM_EN | VOICE_PATH_PLAY);
