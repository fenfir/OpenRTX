 ...
 *
 * Radio-bus I2C1 transport for the HD2 (ck803s + HR_C7000) -- the bus the
 * AT1846S RF transceiver (slave 0xE2) sits on.  SCL=PTA7, SDA=PTA8.
 *
 * PTA7/PTA8 are the pins of the on-chip DesignWare I2C1 controller (base
 * 0x14070000); the SoC pad mux IO_DIPLEX0 bits 7/8 (i2c1_scl_sel/i2c1_sda_sel)
 * select GPIO (=1, bit-bang) vs the I2C1 function (=0).  We drive the HW
 * controller -- LIVE-VERIFIED: an HW-I2C1 read of AT1846S regs
 * 0x00/0x04/0x0a/0x33 returned 0x1846/0x0fd0/0x4c20/0x44a5, byte-for-byte
 ...
 * Build-portable: the bare loader links no-op pthread stubs.
 */
...
/* ===================================================================== *
 *  Hardware DesignWare I2C1 controller @ 0x14070000 (I2C1 accessor from
 *  registers.h).  The register names below alias the I2C_TypeDef fields so
 *  the transfer logic reads exactly as the manual / the proven RTC driver.
 * ===================================================================== */

#define IC_CON (I2C1->IC_CON)           /* master cfg (0x65) */
#define IC_TAR (I2C1->IC_TAR)           /* target slave 7-bit address */
#define IC_DATA_CMD (I2C1->IC_DATA_CMD) /* tx data byte / rx command */
#define IC_FS_SCL_HCNT (I2C1->IC_FS_SCL_HCNT)
#define IC_FS_SCL_LCNT (I2C1->IC_FS_SCL_LCNT)
#define IC_INTR_MASK (I2C1->IC_INTR_MASK)
#define IC_RX_TL (I2C1->IC_RX_TL)
#define IC_TX_TL (I2C1->IC_TX_TL)
#define IC_CLR_TX_ABRT \
    (I2C1->IC_CLR_TX_ABRT) /* read clears the TX_ABRT latch */
#define IC_ENABLE (I2C1->IC_ENABLE)
#define IC_STATUS (I2C1->IC_STATUS)
#define IC_TX_ABRT_SRC \
    (I2C1->IC_TX_ABRT_SRC) /* nonzero = abort latched (NAK/arb-loss) */
#define IC_ENABLE_STATUS (I2C1->IC_ENABLE_STATUS) /* bit0: IC_EN settled */
#define IC_START (I2C1->IC_START) /* HR_C7000-specific transfer trigger */
 ...
 * the bus is idle wedges the controller.  Bounded so a stuck bus can't hang. */
#define I2C1_WAIT_IDLE()                                                     \
    do {                                                                     \
        int _s;                                                              \
        for (_s = I2C1_SPIN_LIMIT; _s && (IC_STATUS & STA_ACTIVITY); --_s) { \
        }                                                                    \
    } while (0)
 ...
 * iterations, so 0x2000 gives ~8x margin while a dead bus costs ~ms. */
 ...
 * transfer failed.  A failed transfer leaves its command bytes rotting in the
 * TX FIFO and they corrupt every transfer after it.  Recovery: clear any abort
 * latch, disable the controller (flushes both FIFOs) and drop the TAR cache so
 * the next transfer re-enables from scratch. */
...
void i2c0_init(void)
{
    ...
    SOCSYS->IO_DIPLEX0 &= ~DIPLEX0_I2C1_PINS;
 ...
 * target actually changes.  For the common single-target case the controller
 * is enabled once and never toggled -- matching the host-verified sequence. */
 ...
 * starting early (mid-load) fragments the transfer. */
...
void i2c0_write(uint8_t addr, void *buf, size_t len, bool sendStop)
{
     ...
     * mid-load fragments the write).  For a write longer than the FIFO, start
     * once the FIFO fills so it drains while we keep feeding. */
    int spin;
    bool started = false;
    i2c1_set_target(addr);
    for (size_t i = 0; i < len; ++i) {
        if (!started
            && !(IC_STATUS & STA_TFNF)) /* FIFO full -> must start to drain */
        {
            IC_START = 1u;
            started = true;
        }
    ...
    if (!started)
        IC_START = 1u; /* whole write fit: start now */
...
{
    /* A 1-byte read; success is non-wedged completion. */
    uint8_t v;
...
{
    pthread_mutex_lock(&i2c0_mutex);
}
void i2c0_releaseDevice(void)
{
    pthread_mutex_unlock(&i2c0_mutex);
}
