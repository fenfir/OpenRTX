...
typedef struct {
    ...
    volatile uint32_t DAC_CONTROL; /* 0x70 codec DAC control (bit31, pwda bit5) */
    volatile uint32_t ADC_CONTROL; /* 0x74 codec ADC control (enadc/enref) */
    uint32_t RESERVED2[2];         /* 0x78 - 0x7c */
    volatile uint32_t
        VOICE_PATH; /* 0x80 voice-source select (bit0=0 -> codec DAC) */
    volatile uint32_t PCM_MODE; /* 0x84 PCM bridge mode */
    volatile uint32_t
    ...
    volatile uint32_t
        AF_GATE; /* 0x39c modem interrupt mask (FM vendor 0x1007f) */
    ...
    volatile uint32_t
        LAYER2_CONTROL;  /* 0x400 DMR L2 [7]txen [6]rxen (FM: RX datapath) */
    uint32_t RESERVED10; /* 0x404 */
    volatile uint32_t LAYER2_TXRX_CTRL; /* 0x408 DMR L2 tx/rx next-slot enable */
...
static_assert(offsetof(SOCSYS_TypeDef, IO_DIPLEX2) == 0x3c,
              "SOCSYS IO_DIPLEX2");
static_assert(offsetof(SOCSYS_TypeDef, DAC_CONTROL) == 0x70,
              "SOCSYS DAC_CONTROL");
static_assert(offsetof(SOCSYS_TypeDef, VOICE_PATH) == 0x80,
              "SOCSYS VOICE_PATH");
...
static_assert(offsetof(SOCSYS_TypeDef, INT_STATUS) == 0x3b0, "SOCSYS INT_STATUS");
...
#define AF_GATE_FM_VENDOR 0x0001007fu /* vendor FM modem interrupt-mask value */

/*
 * CPU -> codec-DAC PCM playback bridge (SAHB shared-SRAM mailbox + frame IRQ).
 * Once armed (PCM_MODE=3, VOICE_PATH bit0 + playback bit) the codec requests
 ...
 */
#define SOFT_RSTN_PCM_BITS      0x18u /* SYS_SOFT_RSTN [4:3]: codec/audio PCM blocks */
#define VOICE_PATH_PCM_EN       0x01u /* VOICE_PATH bit0: 1 = PCM bridge, 0 = direct DAC */
#define VOICE_PATH_PLAY         0x20u /* VOICE_PATH playback-side enable */
#define VOICE_PATH_CAP          0x10u /* VOICE_PATH mic-capture-side enable */
#define INT_STATUS_PCM_PLAY_ACK 0x20u /* INT_STATUS: a playback frame was supplied */
#define INT_STATUS_PCM_CAP_ACK  0x10u /* INT_STATUS: a capture frame was consumed */
...
#define PWM_CH1 ((PWM_Channel_TypeDef *)0x140c0020u) /* beep tone (audio leg) */
...
#define AT1846S_I2C_SLAVE 0x71u             /* AT1846S 7-bit address on I2C1 */
